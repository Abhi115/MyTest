...
public class C4CAppPage extends BasePage {
	...
	_waitForJStoLoad();
	driver.findElement(getTechincalComplexity("SPR Status")).click();
	sleepExecution(2);
	clickOn(driver.findElement(getElementDropDownValue("Within DoA and No SPR required")));
	waitForAjaxRequestsToComplete();
	
	click(driver.findElement(getGoApproval("Commercial Approved")));
	waitForAjaxRequestsToComplete();
	
	click(driver.findElement(getGoApproval("Sales Approved")));
	waitForAjaxRequestsToComplete();
	
	click(driver.findElement(getGoApproval("Solution Approved")));
	waitForAjaxRequestsToComplete();
