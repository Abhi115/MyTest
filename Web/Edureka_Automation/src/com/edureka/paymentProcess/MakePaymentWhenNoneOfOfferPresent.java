...
public class MakePaymentWhenNoneOfOfferPresent extends DriverTestCase{
	...
	public void test_013MakePaymentWhenNoneOfOfferPresent() throws Exception {
		...
		try {
			...
			//Verify data in pre-order table after course enrollment
			addLog("Verify data in pre-order table after course enrollment");
			...
			myProfilePage=myProfilePage.dataVerificationInUser_PreOrderTable(course_Id,course_Title,paymentGateway,currency,"MakePaymentWhenNoneOfOfferPresent","pre_orders_RazorPayment");
			...
			addLog("Verify the Url  on my profile page after payment");
			...
			addLog("Verify data in event table");
			myProfilePage=myProfilePage.dataVerificationInUser_Events_Table_PaymentType(eventType);
	...
	public void test_019BackendDatabaseValidationWhenNoOfferPresent() throws Exception {
		try {        	
			...
			//Data Verification in pre orders table
			addLog("Verify Data in Pre-Order table");
			myProfilePage=myProfilePage.dataVerificationInUser_PreOrderTable(course_Id,course_Title,paymentGateway,currency,"MakePaymentWhenNoneOfOfferPresent","pre_orders_PapalPayment");

			// Verify Data in User Course Table
			String course_Group = propertyReader.readTestData("Course_Group_ApacheSpark&Scala");
			String isPaidValue= propertyReader.readTestData("PaidCourses_Is_Paid_Value");
			addLog("Verify Data in User Course Table");
			myProfilePage=myProfilePage.dataVerificationInUser_CoursedTable(course_Id, isPaidValue, course_Group);

			// Database verification in Post order table
			addLog("Verify Data in in Post order table");
			myProfilePage=myProfilePage.dataVerificationInUser_PostOrderTable(course_Id,course_Title,paymentGateway,currency);

			// Database verification in Post order table
			addLog("Verify Data in Post order table on the basis of pre order table");
