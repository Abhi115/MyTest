...
public class CalculateAndValidatePricesOnChangeOfCurrency extends DriverTestCase{
    ...
    @Test
    public void test_003CalculateAndValidatePricesOnChange() throws Exception {
        ...
        try {
            ...
            orderSummaryPage=orderSummaryPage.dataVerificationInUser_PreOrderTableForINR(course_Id,course_Title);
            ...
            orderSummaryPage=orderSummaryPage.dataVerificationInUser_CoursedTable(course_Id, isPaidValue, course_Group);

            // Change currency
            currency = propertyReader.readTestData("USDCurrency");
            addLog("Change currency from currency dropdown");
            ...
            // Verify Data in Pre-Order table for USD
            addLog("Verify Data in Pre-Order table for USD");
            orderSummaryPage=orderSummaryPage.dataVerificationInUser_PreOrderTableForUSD(course_Id,course_Title);

        }
        catch (final Error e) {
            captureScreenshot("test_003CalculateAndValidatePricesOnChange");
            throw e;
